...
if (isset($_REQUEST['action']) && $_REQUEST['action']='sendsched') {
  ...
  $date  = date('Y-m-d');
  $date0 = date('Y-m-d', mktime(0, 0, 0, date('m'), date('d')+$w1, date('Y')));
  $date1 = date('d.m', mktime(0, 0, 0, date('m'), date('d')+$w1, date('Y')));
  ...
  $sql = "SELECT a.`class_id`, l.* , d.discipline, a.discipline_id, a.teacher_id, a.date_schedule, a.order_schedule, a.id_schedule"
        ...
        ." AND (date_format( DATE_ADD( NOW() , INTERVAL ".$w1." DAY ) , '%Y-%m-%d' ) = l.`lesson_date` )"
...
//       ." LEFT JOIN `".TABLE_SUBJECTS."`    AS s ON (s.discipline_id = a.discipline_id) AND (s.class_id = a.class_id) AND ( s.teacher_id = a.teacher_id )"
        ." WHERE ( a.date_schedule = date_format(DATE_ADD( NOW() , INTERVAL ".($w1-1)." DAY ) , '%w' ) )"
        ." AND (a.quarter_id='".$quarter_id."')"
        ." AND ( date_format(DATE_ADD( NOW() , INTERVAL ".$w1." DAY ), '%Y-%m-%d' ) BETWEEN a.started AND a.finished )"
...
<table id="edit">
  ...
  <tr>
    <td><?php echo $lang['sms_send_sched'];?>: <?php echo date('d.m.Y', mktime(0,0,0,date('m'), date('d')+$w1, date('Y'))); ?> </td>
  </tr>
  
  <tr>
   <td>
   <form action="sms_sending-schedull.php" method="post">
