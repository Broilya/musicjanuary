...
<?php
  ...
  else 
  {
      ...
      if ($_GET['t'] == "quater") {
          $q = "select q.quarter_name, y.started, y.finished from quarters as q
 join school_years as y on y.school_year_id=q.school_year_id
...
<?php
        ...
        foreach ($arr as $discipline => $grade) {
            ...
            echo "<td>" . round($year, 1)."</td><td>{$exam}</td><td>{$itog}</td></tr>";
        }
