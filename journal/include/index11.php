...
// config
$db_config = mysql_query("SELECT * FROM config");
...
if (isset($_GET['txt']) && isset($_GET['pref'])) {
 ...
 if(!isset($_POST['action'])) {$_POST['action'] = '';}
  ...
  if ($_POST['action'] == 'update_photo' && $_FILES['uploadfile']['tmp_name']!=='') {
    ...
    @unlink("student_photo/".$_POST['photo']);
    @unlink("student_photo/sm/".$_POST['photo']);
    $student_photo = UploadedPhotoFile("student_photo/");		 
    $query = mysql_query("UPDATE students SET photo='".$student_photo."', update_photo=".($_POST['update_photo']+1)." WHERE student_id='".$_POST['student_id']."'"); 	
    header('Location: index.php');
    exit();        
  	...
  	$student_photo = UploadedPhotoFile("student_photo/");	
    $query = mysql_query("UPDATE students SET photo='".$student_photo."' WHERE student_id='".$_POST['student_id']."'"); 	
    header('Location: index.php');
    exit();         
  }
...
<table   height="100%" id="oblozhka" width="100%" >
...
$dates = array();
$res = db_query('SELECT value_config FROM config WHERE key_config="DAYS"');
...
<?php
  ...
  $res = db_query('SELECT class_id FROM students_in_class WHERE student_id='.$student_id);
  $class_id = array_pop(mysql_fetch_assoc($res));
  $disceplines = get_disciplines_from_class($class_id);
  
  $res = db_query("SELECT students_in_class.student_id AS student_id, students_on_lesson.grade AS grade, students_on_lesson.behavior AS behavior, lessons.lesson_date AS lesson_date, lessons.dz AS lesson_dz, lessons.topic AS lesson_topic, lessons.lesson_id AS lesson_id, subjects.discipline_id AS discipline_id, disciplines.discipline AS discipline FROM ((((students_in_class join subjects on((subjects.class_id = students_in_class.class_id))) join lessons on((lessons.subject_id = subjects.subject_id))) left join students_on_lesson on(((students_on_lesson.student_id = students_in_class.student_id) and (students_on_lesson.lesson_id = lessons.lesson_id)))) left join disciplines on((disciplines.discipline_id = subjects.discipline_id))) WHERE students_in_class.student_id=".$student_id." order by lessons.lesson_date, students_in_class.student_id");
  //$res = db_query('SELECT * FROM viewstudentonlessons WHERE grade IS NOT NULL AND student_id='.$student_id);
  $grades = array();
...
text-shadow:1px 1px 1px #568F23;">'.$discepline.'</font></td>';
    foreach ($dates as $date) {
     ...
     //выводим четвертную оценку
      //if($months[$month]==$months[$fd]){
        $stud=mysql_query("SELECT * FROM students_on_lesson WHERE  `student_id` = ".$student_id." AND `quater`= ".$quarter['quarter_id']." AND `subj_id`=".$discepline_id."");
        $sum=0;
      ...
      if(mysql_num_rows($stud)!=0){
        $stud=mysql_query("SELECT *, avg( grade ) FROM students_on_lesson WHERE  `student_id` = ".$student_id." AND  `subj_id`=".$discepline_id." group by quater");
        $sum=0;
      // echo "SELECT * FROM students_on_lesson WHERE  `student_id` = ".$student_id." . AND `subj_id`=".$discepline_id." group by quater";
      if(mysql_num_rows($stud)!=0){
        for($i=0;$i<mysql_num_rows($stud);$i++){
   ...
   //выводим экзамен
      if(mysql_num_rows($stud)!=0){
        $stud=mysql_query("SELECT * FROM students_on_lesson WHERE  `student_id` = ".$student_id." and `quater`= 10000 AND  `subj_id`=".$discepline_id."");
        $sum=0;
      // echo "SELECT * FROM students_on_lesson WHERE  `student_id` = ".$student_id." `quater`= 100 AND  `subj_id`=".$discepline_id."";
      if(mysql_num_rows($stud)!=0){
        for($i=0;$i<mysql_num_rows($stud);$i++){
            $sum+=mysql_result($stud,$i,2);
        }
...
function dima_mozg($quarter, $started, $fineshed,$fd){
  ...
  $month = intval($started[1]);
   ...
   $subj=db_query('SELECT * FROM disciplines');
   $disciplines=array();
   ...
   $res = db_query('SELECT class_id FROM students_in_class WHERE student_id='.$student_id);
   $class_id = array_pop(mysql_fetch_assoc($res));
   $q=db_query("SELECT * FROM quarters");
   $quarters=array();
   ...
   foreach($quarters as $quarter){
     ...
     //Ищем предметы
     $pred=db_query("SELECT * FROM subjects WHERE class_id=".$class_id."");
     while($p=mysql_fetch_assoc($pred)){
       print "&nbsp;&nbsp;".$disciplines[$p['subject_id']-1]['discipline']."<br>";
     //
        $grades=db_query("SELECT * FROM students_on_lesson WHERE student_id=".$student_id." AND quater='".$q_id."' AND subj_id='".$p['subject_id']."'");
...
if($student['photo'] !=='') { echo "<td><a class=\"highslide\" onclick=\"return hs.expand(this)\" href=\"student_photo/$student[photo]\" title=\"Увеличить фото\"><img src='student_photo/sm/$student[photo]' border=\"0\"></a>".$link_photo."</td><td width=5>&nbsp;</td>"; }
else
{
	echo "<td><img src='images/nophoto.gif' border=1 height='100' width='100'>".$link_photo."</td><td width=5>&nbsp;</td>";
}
$query="SELECT class,letter FROM `students_in_class`  as sic
join  classes as c on c.class_id=sic.class_id
