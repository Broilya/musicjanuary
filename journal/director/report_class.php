...
<?php
 ...
 if ($_REQUEST['disp_id']!="") { $dis_query=" and  d.discipline_id={$_REQUEST['disp_id']} "; } 
  ...
  while($row=mysql_fetch_assoc($res)) {
  	...
  	$stud_grades[$row['discipline']]['id']=$row['discipline_id'];
  $stud_grades[$row['discipline']][]=$row['grade'];
 ...
 foreach($stud_grades as $key => $student) {
  ...
  $aver=0;
  echo "<tr align='center'><td>$n</td><td nowrap=\"nowrap\" align='left'><a href='report_disp.php?disp_id={$student['id']}'>{$key}</a></td>";
