  ...
  $headers .= 'Content-Type: text/plain; charset=UTF-8' . "\r\n";
   ...
   $teacher_class = $teacher['class_id'];
	...
	while ($student_id = mysql_fetch_assoc($res)) {
		...
		$text = "������ $student[last_name] $student[first_name] $student[middle_name] �� ".$_REQUEST['date1']."-".$_REQUEST['date2']." ������� ��������� ������:\n";
		$res2 = db_query("SELECT grade, discipline"
                  ...
                  ." WHERE lessons.lesson_date >= '".implode('-', array_reverse(explode('.', $_REQUEST['date1'])))."'"
                  ." AND lessons.lesson_date <= '".implode('-', array_reverse(explode('.', $_REQUEST['date2'])))."'"
                  ." AND student_id=$student_id;");
   while($grade = mysql_fetch_assoc($res2)) {
    $text .="$grade[discipline]: $grade[grade]\n";
  }
  if ($student['email'] != '') {
  	mail($student['email'], "������ ������� $student[last_name] $student[first_name] $student[middle_name] �� ".$_REQUEST['date1']."-".$_REQUEST['date2'], $text, $headers);
  }
	}
...
<br />
      ...
      <h2><p><b> <?php  echo $lang['excell_upload'];?> </b></p></h2>
