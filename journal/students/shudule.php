...
<?php
  ...
  $res = db_query("SELECT class_id FROM `".TABLE_STUDENTS_IN_CLASS."` WHERE student_id='".$student_id."' LIMIT 0,1");
  $class_id = array_pop(mysql_fetch_assoc($res));

  $disceplines = get_disciplines_from_class($class_id);
$dump=1;if ($dump) {echo "\n".'|$disceplines='."\n";print_r($disceplines);}
  
  $res = db_query($sql="SELECT sc.student_id AS student_id, sl.grade AS grade, sl.behavior AS behavior, l.lesson_date AS lesson_date, l.dz AS lesson_dz, l.topic AS lesson_topic, l.lesson_id AS lesson_id, s.discipline_id AS discipline_id, d.discipline AS discipline"
...
//if($months[$month]==$months[$fd]){
    ...
    if (date("Y-m-d")>$last_day['finished']) {
      if(mysql_num_rows($stud)!=0){
        ...
        $sum=0;
      // echo "SELECT * FROM `".TABLE_STUDENTS_ON_LESSON."` WHERE  `student_id` = ".$student_id." . AND `subj_id`=".$discepline_id." group by quater";
        if(mysql_num_rows($stud)!=0){
          for($i=0;$i<mysql_num_rows($stud);$i++){
 ...
 //выводим экзамен
    if(mysql_num_rows($stud)!=0){
      ...
      $sum=0;
    // echo "SELECT * FROM `".TABLE_STUDENTS_ON_LESSON."` WHERE  `student_id` = ".$student_id." `quater`= 100 AND  `subj_id`=".$discepline_id."";
      if(mysql_num_rows($stud)!=0){
        for($i=0;$i<mysql_num_rows($stud);$i++){
          $sum+=mysql_result($stud,$i,2);
        }
...
function dima_mozg($quarter, $started, $fineshed,$fd){
  ...
  if($student['photo'] !=='') { 
    echo "<td><a class=\"highslide\" onclick=\"return hs.expand(this)\" href=\"../student_photo/".SUBDOMEN."/$student[photo]\" title=\"Увеличить фото\"><img src='../student_photo/".SUBDOMEN."/sm/$student[photo]' border=\"0\"></a>".$link_photo."</td><td width=5>&nbsp;</td>"; 
  } else  {
