...
<div style="float: left;width: 120px;padding:20px;"></div>
...
<option value=""><?php echo $lang['select'];?></option>
 ...
 {
     $ch=$row['current']? " selected ":"";
 	echo "<option value='{$row['school_year_id']}' $ch>{$row['name_year']}</option>";
 	$ch="";
 }
   ?>
 </select>
 ...
 <option value=""><?php echo $lang['select'];?></option> 
   <?php
   $db_classes = mysql_query("SELECT * FROM `".TABLE_SCHOOL_QUARTERS."` where school_year_id=(select school_year_id FROM `".TABLE_SCHOOL_YEARS."` where current=1 ) and quarter_type=1");
