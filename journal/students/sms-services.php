...
{
  $sql = "SELECT a.student_id, a.pin_code, b.grade, d.discipline, c.lesson_date FROM `".TABLE_USERS_STUDENTS."` AS a LEFT JOIN `".TABLE_STUDENTS_ON_LESSON."` AS b ON b.student_id=a.student_id LEFT JOIN `".TABLE_LESSONS."` AS c ON c.lesson_id=b.lesson_id LEFT JOIN `".TABLE_SUBJECTS."` AS s ON s.subject_id=c.subject_id LEFT JOIN `".TABLE_SPR_DISCIPLINES."` AS d ON d.discipline_id=s.discipline_id WHERE pin_code='".$pincod."' AND lesson_date >='".date('Y-m-d', mktime(0,0,0, date('m'), date('d')-7, date('Y')))."' AND lesson_date <='".date('Y-m-d')."'";
  $res = db_query($sql);
...
{
  $sql = "SELECT a.student_id, a.pin_code, c.dz, c.lesson_date, d.discipline, s.class_id FROM `".TABLE_USERS_STUDENTS."` AS a LEFT JOIN `".TABLE_STUDENTS_IN_CLASS."` AS b ON b.student_id=a.student_id LEFT JOIN `".TABLE_SUBJECTS."` AS s ON s.class_id=b.class_id LEFT JOIN `".TABLE_LESSONS."` AS c ON c.subject_id=s.subject_id LEFT JOIN `".TABLE_SPR_DISCIPLINES."` AS d ON d.discipline_id=s.discipline_id WHERE pin_code='".$pincod."' AND lesson_date ='".date('Y-m-d')."'";
  $res = db_query($sql);
