...
if(isset($_REQUEST['action'])) {
  ...
  elseif ($_REQUEST['action']=="edit_grades" and $_REQUEST['sub_act']=='zero') {
    ...
    $sql = "UPDATE `".TABLE_LESSONS."` SET active=(SELECT if((`date_grade` <> '') AND (DATE_ADD( `date_grade`, INTERVAL 2 DAY ) < NOW()) , 0, 1)"
...
//            db_query($sql);
    ...
    exit(header ('Location: theme-lessons.php?&lesson_id='.$_lesson_id.'&subject_id='.$subject_id.'&action='.$_REQUEST['action']));
...
<br />
<form action="theme-lessons.php"  method="post" id="ftheme">
  ...
  <div id="errordiv"></div>
   ...
   onChange="
     if (this.form.classes.value+0 > 0) {
       location = 'theme-lessons.php?classes='+this.form.classes.value+'&lesson_date='+this.value;
     }
   "/>
...
/*$(document).ready(function(){ */
  ...
  <select name="classes" onChange="
     if (this.form.lesson_date.value != '') {
       location = 'theme-lessons.php?classes='+this.value+ '&lesson_date='+this.form.lesson_date.value;
     }
   ">
  <option value=""><?php echo $lang['select'];?></option>
<?php 

/*       ewd_getcontent('theme-lessons.php?date='+this.value+'&class=<?php echo $class_id;?>', 'errordiv');*/
...
//echo "SELECT subject_id FROM `".TABLE_SUBJECTS."` WHERE discipline_id='".$_SESSION['discipline']."' AND teacher_id='".$_SESSION['teacher']."' AND class_id = '".$_SESSION['classes']."'";
  $sql = "SELECT s.*, b.discipline, t.`last_name`, t.`first_name`, t.`middle_name`"
        ." , s.subject_id, l.lesson_id ,l.topic, l.dz, l.schedule_id, a.id_schedule"
...
<input type='hidden' name='subject_id' value='<?php print $subject_id; ?>' /></form><br/>
  ...
  <form action="theme-lessons.php" method="post" name="main">
...
<tr><td valign="top" align="center">
  ...
  <tbody>
    <tr><td><?php echo $day."<br>".implode('.', array_reverse(explode('-', $_REQUEST['lesson_date'])));?></td>
        <td><div id="teacher"><?php echo $lesson['discipline'];?><br><?php echo $lesson['last_name'];?></div></td>
...
*/
  ...
  $sql = "SELECT a.*, a.id_schedule AS schedule_id, b.discipline, t.`last_name`, t.`first_name`, t.`middle_name`"
        ." , s.subject_id, l.lesson_id ,l.topic, l.dz, g.short"
        ." FROM `".TABLE_SCHEDULE."` AS a"
//         ." LEFT JOIN `".TABLE_SUBJECTS."`    AS s ON (s.discipline_id = a.discipline_id) AND (s.class_id = a.class_id) AND ( s.teacher_id = a.teacher_id )"
//         ." LEFT JOIN `".TABLE_LESSONS."`     AS l ON (l.subject_id = s.subject_id)       AND (l.lesson_date = '".$lesson_date."')"
...
if ($dump) print_r($schedule);
    ...
    $alt_group      = (empty($schedule['short'])) ? '' : ' ['.$schedule['short'].']';
    $schedule_group = (empty($schedule['short'])) ? '' : ' <span style="font-size:9pt;color:gray;">'.$alt_group.'</span>';
?>
  <tr>
    ...
    onClick="
     ...
     if (document.getElementById('lesson_id').value != '') {
       location= 'theme-lessons.php?lesson_date=<?php echo $_date;?>&lesson_id='+document.getElementById('lesson_id').value;
       return false;
     } else {
       location= 'ajaxform-lesson.php?schedule_id=<?php echo $schedule['schedule_id'];?>&lesson_id=<?php 
                  echo $schedule['lesson_id'];?>&subject_id=<?php echo $schedule['subject_id'];?>';
       return false;
       return true;
     }
       return false;
    "  
    href='';
    title="<?php echo $schedule['discipline'].$alt_group;?>" cl_ass="thickbox"><?php echo $schedule['discipline'].$schedule_group;?></a></td>
  </tr>
...
//         document.write('<a href="edit_lesson.php?subject_id='+subject_id+'&lesson_id='+lesson_id+'&TB_iframe=true&height=300&width=400&<?php echo uniqid('r');?>" title="Редактировать урок" class="thickbox"><?php echo $lang['edit'];?></a></td><td><input value="1" type="checkbox" name="lesson_id">');
  ...
  <tr><td valign="top" align="center">
  <!--form action="theme-lessons.php" method="post">
...
$dump=0;if ($dump) echo $sql."\n";
  ...
  while ($row = mysql_fetch_assoc($res)) {
    ...
    echo "<th  style='writing-mode:tb-rl' colspan='3'><a href='theme-lessons.php?subject_id=".$subject_id
...
$dump=0;if ($dump) {echo "|_lesson_id=".$_lesson_id.'<br><pre>$grades=|';print_r($grades);echo '</pre>|<pre>$lessons=|';print_r($lessons);echo '</pre><br>';}
