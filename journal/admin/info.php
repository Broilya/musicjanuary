...
<table>
  ...
  <tr>
    ...
    {
      echo '<option value="teacher" selected>'.$lang['inform_teacher'].'</option>';
    }
    else
    {
   	  echo '<option value="teacher">'.$lang['inform_teacher'].'</option>';
    }
    
    if ($information['information_section'] == 'student')
    {
      echo '<option value="student" selected>'.$lang['inform_student'].'</option>';
    }
    else
    {
    ...
    if ($information['information_section'] == 'balance')
    {
      echo '<option value="balance" selected>'.$lang['inform_balance'].'</option>';
    }
    else
    {
   	  echo '<option value="balance">'.$lang['inform_balance'].'</option>';
    }
	 echo '</select></td>
  </tr>
  <tr>
   ...
   $db_classes = mysql_query("SELECT class_id, class, letter FROM `".TABLE_CLASSES."` WHERE school_year =(select school_year_id FROM `".TABLE_SCHOOL_YEARS."` where current=1)");
