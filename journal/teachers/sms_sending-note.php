...
//die("|".$OFF."|");
  ...
  $date=date('Y-m-d');
if (isset($_REQUEST['action']) && $_REQUEST['action']=='sendnote') {
		
  $sql = "SELECT s.last_name, s.student_id, mother_cell_phone, father_cell_phone, mode , sol.*, l.*, d.*"
          ...
          ." AND NOT ifnull( sol.date_note, 0 )";
...
if (mysql_errno()) { echo $sql.'<br>'. mysql_errno().':'.mysql_error().'<br>'; }
  ...
  while($row = mysql_fetch_assoc($res2)) {
    ...
    $array[$row['student_id']]['notes'][$row['discipline']][]=$row['note'];
...
if ($dump) print_r($array);
  ...
  foreach ($array as $student_id=>$data) 
  {
    ...
    foreach ($data['notes'] as $disp=>$notes) 
    {
...
if ($dump) echo "|1) data['m_fone']=".$data['m_fone']."|";
...
if ($dump) echo "|2) data['m_fone']=".$data['m_fone']."|";
...
if ($dump) echo "|1) data['f_fone']=".$data['f_fone']."|";
...
if ($dump) echo "|2) data['f_fone']=".$data['f_fone']."|";
...
//echo $data['m_fone'];
    ...
    if (!empty($data['m_fone']) || !empty($data['f_fone'])) {
      ...
      foreach($data['studless_id'] as $key=>$studless_id) {
        $sql = "UPDATE `".TABLE_STUDENTS_ON_LESSON."` SET `date_note`=NOW()"
...
if (mysql_errno()) { echo $sql.'<br>'. mysql_errno().':'.mysql_error().'<br>'; }
      ...
      $sql = "INSERT INTO `".TABLE_SMS_LOGS."` (`studless_id`, `student_id`, `date`, `type`, `text`)"
            ." VALUES ('".$studless_all."', '".$student_id."', NOW(), 'z', '".$phones.$test_to_send."');";
if ($dump) echo $sql;
      $res=mysql_query($sql);
if (mysql_errno()) { echo $sql.'<br>'. mysql_errno().':'.mysql_error().'<br>'; }
    }
  ...
  $send_OK =  "\n<br>".$lang['sms_notes_send_ok']."(".$sms_count.")";
...
$nums_note = db_get_cell("SELECT COUNT(1) FROM `".TABLE_STUDENTS_ON_LESSON."` sl"
                        ." INNER JOIN `".TABLE_LESSONS."` l ON l.lesson_id=sl.lesson_id"
                        ." WHERE l.lesson_date='".date('Y-m-d')."' AND sl.note<>'';");
...
<table id="edit">
  <tbody>
  <tr class="TableHead">
    <th colsapan="2">{$lang['sms_notes_send']}</th>
  </tr>
  <tr>
    <td>{$send_OK}</td>
  </tr>
...
<br>";
  ...
  echo "<tr>
    <td>За {$date} сделано {$nums_note} замечаний.</td>
  </tr>";
?>
  <tr>
	 <td>
<link type="text/css" href="../css/jquery-ui.css" rel="stylesheet" />
	 ...
	 <form action="sms_sending-note.php" method="post">
	 <input type="hidden" name="action" value="sendnote" />
    <?php echo $lang['sms_send_commit_notes'];?> 
