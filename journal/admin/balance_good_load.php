...
if (isset($_REQUEST['action']) && $_REQUEST['action']='sendgrade') {
  ...
  $headers .= 'Content-Type: text/plain; charset=UTF-8' . "\r\n";
   ...
   $teacher_class = $teacher['class_id'];
	...
	while ($student_id = mysql_fetch_assoc($res)) {
		...
		$student = db_get_first_row("SELECT a.*, b.class_id FROM `".TABLE_USERS_STUDENTS."` AS a LEFT JOIN `".TABLE_STUDENTS_IN_CLASS."` AS b ON b.student_id = a.student_id WHERE a.student_id=$student_id AND b.class_id = $teacher_class;");
  ...
  ." WHERE lessons.lesson_date >= '".implode('-', array_reverse(explode('.', $_REQUEST['date1'])))."' AND lessons.lesson_date <= '"
  .implode('-', array_reverse(explode('.', $_REQUEST['date2'])))."' AND student_id=$student_id;");
...
<div align="center"> 
<br />
<h2><p><b> <?php echo $lang['balance_up_good'];?> </b></p></h2>
</table>
