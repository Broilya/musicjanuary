...
<?php  $teacher_class = db_get_first_row("SELECT class, letter FROM `".TABLE_CLASSES."` WHERE teacher_id='".$_SESSION['teacher_id']."'");
  		echo $teacher_class['class']. "-".$teacher_class['letter']; ?><br> 
  
<span class="head_top">Четверть:</span>
    <form action="report.php">
    <select name='id_quater' onChange="javascript: this.form.submit();"><option value='<?php print $quater_id ?>'><?php print $list_quater['quarter_name']; ?></option>
    <?php
    ...
    foreach($list_quater as $quater){
      if($quater['quarter_name']!=$tek_quarter){
...
<?php
  ...
  $query="SELECT s.student_id, s.last_name, s.first_name,s.middle_name, s.photo, s.address, s.phone, sol.grade"
        ...
        ." ALTER TABLE `".TABLE_STUDENTS_IN_CLASS."` as sic on sic.student_id=sol.student_id"
        ." ALTER TABLE `".TABLE_CLASSES."` as cl on cl.class_id=sic.class_id"
        ." ALTER TABLE `".TABLE_USERS_STUDENTS."` as s on s.student_id=sol.student_id"
...
</td><td valign='top'><div align='left' width='20'>
<span class="head_top">Информация - Сообщение учителям</span>
<table id="rounded-corner2" >
  <thead>
  <tr>
    <th class="rounded-left" width="10">Дата</th>
    <th class="rounded-right" width="10">Тема</th>
  </tr>
  </thead>
  <tbody>
<?php
  ...
  {
  	echo "<tr><td width='100'>$information[information_date]</td>
    <td><a href='information.php?view=$information[information_id]'>$information[information_title]</a></td></tr>";
   }
  }
?>
</tbody>
