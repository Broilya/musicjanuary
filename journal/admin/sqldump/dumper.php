...
// Передача данных для глобальной статистики
  ...
  function restore(){
    if(is_uploaded_file($_FILES['upload']['tmp_name'])) {
      ...
      $this->main();
      echo tpl_l("Файл ".$_FILES['upload']['name']." закачен");
      exit;
    }

    if (!isset($_POST)) {$this->main();}
    set_error_handler("SXD_errorHandler");
    $buttons = "<INPUT ID=back TYPE=button VALUE='Вернуться' DISABLED onClick=\"location=location;if (0) history.back();\">";
...
// Вставляем кодировку для таблиц, если она не указана и установлена auto кодировка
            ...
            if ($last_showed != $table) {
              $cache .= tpl_l("Таблица `{$table}`."); $last_showed = $table;
            }
        }
...
<TR>
<TD WIDTH=35%>Закачать:</TD>
<TD WIDTH=65%><INPUT NAME=upload type=file style="width:100%;">
</TD>
</TR>
<TR><TD COLSPAN=2><DIV ID=logarea STYLE="display:none;width: 100%; height: 14px; border: 1px solid #7F9DB9; padding: 3px; overflow: auto;"></DIV></TD></TR>
</TABLE>
</FIELDSET>
</SPAN>
<SCRIPT>
