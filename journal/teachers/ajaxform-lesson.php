...
<script language='javascript'> <!--
   location= 'lessons.php?lesson_date=".$_date."&lesson_id=".$lesson_id."';
*/
//--></script>";
//    header('Location: lessons.php?lesson_date='.$_date.'&lesson_id='.$lesson_id);
//http://schoole.servicetelematic.ru.int/journal/admin/lessons.php?subject_id=36&act=edit_grades&sub_act=zero&less=104
/*
?>
<script language='javascript'> <!--
self.parent.tb_remove();
self.parent.location= 'lessons.php?subject_id=<?php echo $subject_id;?>&act=edit_grades&sub_act=zero&lesson_id=<?php echo $lesson_id;?>&less=<?php echo $lesson_id;?>';
//--></script>
	
<?php
    exit;
*/
    header('Location: lessons.php?subject_id='.$subject_id.'&act=edit_grades&sub_act=zero&lesson_id='.$lesson_id.'&less='.$lesson_id);
    exit;
    $mode = 'success_update';
  } else {
/*
...
<script language='javascript'> <!--
self.parent.tb_remove();
self.parent.location= 'lessons.php?lesson_date=<?php echo $_date;?>&lesson_id=<?php echo $lesson_id;?>';
//--></script>
...
//  http://schoole.servicetelematic.ru.int/journal/admin/lessons.php?subject_id=36&act=edit_grades&sub_act=zero&less=289  
  if ($mode == 'success_update') {
  	echo $lang['new_lesson'].'<br /><br />';
  	echo '<input type="button" value="&nbsp;&nbsp;'.$lang['close'].'&nbsp;&nbsp;" onclick="self.parent.tb_remove();
  	self.parent.location= \'lessons.php?subject_id='.$subject_id.'&act=edit_grades&sub_act=zero&lesson_id='.$lesson_id.'&less='.$lesson_id.'\';
  	" />';
  } elseif ($mode == 'update') {
